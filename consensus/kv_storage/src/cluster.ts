...
const initMokka = async () => {
  ...
  const resMiddleware = async (packet: ExtendedPacketModel, peerPublicKey: string): Promise<ExtendedPacketModel> => {
    packet.logIndex = logsStorage.length;
    const peerIndex = knownPeersState.get(peerPublicKey) || -1;

    if (mokka.state === NodeStates.LEADER && packet.type === MessageTypes.ACK && peerIndex < logsStorage.length && logsStorage.length !== 0) {
      packet.data = logsStorage[peerIndex + 1];
      console.log('res', peerIndex, knownPeersState, peerPublicKey)
    }
...
const getLog = async (mokka, key) => {
  const valIndex = logsStorage.findIndex(i=> i.key === key);

  if(valIndex === -1){
    return;
  }

  mokka.logger.info(`index: ${valIndex}, value: ${logsStorage[valIndex].value}`);
};
